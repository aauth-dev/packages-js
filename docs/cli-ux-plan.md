...
2. **`sign-token` → `token` — suggested (mirrors `gh auth token`).** Today's command is `sign-token`; suggest renaming it to `token`, since it mirrors `gh auth token`. A standalone command to mint an agent token for use elsewhere (curl, debugging).
